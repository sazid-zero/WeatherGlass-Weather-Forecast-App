...
export function WeatherStatsGrid(_a) {
    var weatherData = _a.weatherData, _b = _a.className, className = _b === void 0 ? "" : _b;
    var uvIndexInfo = getUVIndexLevel(weatherData.uvIndex || 0);
    var airQualityInfo = getAirQualityLevel(weatherData.airQuality || 1);
    var windDirection = getWindDirection(weatherData.windDirection);
    var stats = [
        {
            id: 'feels-like',
            title: 'Feels Like',
            value: "".concat(Math.round(weatherData.feelsLike), "\u00B0"),
        ...
        },
        {
            id: 'humidity',
            title: 'Humidity',
            value: "".concat(weatherData.humidity, "%"),
        ...
        },
        {
            id: 'wind',
            title: 'Wind Status',
            value: "".concat(weatherData.windSpeed, " m/s"),
            description: "".concat(windDirection),
            icon: Wind,
        ...
        },
        {
            id: 'visibility',
            title: 'Visibility',
            value: "".concat((weatherData.visibility / 1000).toFixed(1), " km"),
    ...
    return (_jsxs("div", { className: "weather-stats-grid grid grid-cols-2 sm:grid-cols-2 xl:grid-cols-3 gap-6 ".concat(className), children: [stats.map(function (stat, index) { return (_jsxs(motion.div, { className: "glass-card-hover rounded-3xl p-6 bg-gradient-to-br  ".concat(stat.color, " col-span-2 sm:col-span-1"), initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.6, delay: index * 0.1 }, whileHover: { scale: 1.02 }, children: [_jsx("div", { className: "flex items-start justify-between mb-4", children: _jsxs("div", { className: "flex items-center space-x-3", children: [stat.customIcon ? (_jsx(stat.customIcon, { className: "w-6 h-6 ".concat(stat.iconColor) })) : (_jsx(stat.icon, { className: "w-6 h-6 ".concat(stat.iconColor) })), _jsx("h3", { className: "font-semibold text-foreground", children: stat.title })] }) }), _jsxs("div", { className: "space-y-2", children: [_jsx(motion.p, { className: "text-3xl font-bold text-foreground", initial: { scale: 0.5 }, animate: { scale: 1 }, transition: { duration: 0.5, delay: index * 0.1 + 0.3 }, children: stat.value }), _jsx("p", { className: "text-sm text-muted-foreground", children: stat.description })] }), stat.id === 'humidity' && (_jsx("div", { className: "mt-4", children: _jsx("div", { className: "w-full bg-white/20 rounded-full h-2", children: _jsx(motion.div, { className: "bg-blue-400 h-2 rounded-full", initial: { width: 0 }, animate: { width: "".concat(weatherData.humidity, "%") }, transition: { duration: 1, delay: index * 0.1 + 0.5 } }) }) })), stat.id === 'uv-index' && (_jsx("div", { className: "mt-4", children: _jsx("div", { className: "w-full bg-white/20 rounded-full h-2", children: _jsx(motion.div, { className: "h-2 rounded-full ".concat((weatherData.uvIndex || 0) <= 2 ? 'bg-green-400' :
                                    (weatherData.uvIndex || 0) <= 5 ? 'bg-yellow-400' :
                                        (weatherData.uvIndex || 0) <= 7 ? 'bg-orange-400' :
                                            (weatherData.uvIndex || 0) <= 10 ? 'bg-red-400' :
                                                'bg-purple-400'), initial: { width: 0 }, animate: { width: "".concat(Math.min(((weatherData.uvIndex || 0) / 12) * 100, 100), "%") }, transition: { duration: 1, delay: index * 0.1 + 0.5 } }) }) }))] }, stat.id)); }), _jsxs(motion.div, { className: "glass-card-hover rounded-3xl p-6 bg-gradient-to-br from-amber-400/20 to-orange-400/20 md:col-span-2 col-span-2", initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.6, delay: 0.8 }, whileHover: { scale: 1.02 }, children: [_jsxs("h3", { className: "font-semibold text-foreground mb-6 flex items-center space-x-2", children: [_jsx(Sun, { className: "w-5 h-5 text-yellow-500" }), _jsx("span", { children: "Sun & Moon" })] }), _jsx("div", { className: "grid grid-cols-2 gap-6", children: sunData.map(function (sun, index) { return (_jsxs("div", { className: "text-center", children: [_jsx(motion.div, { className: "flex justify-center mb-3", initial: { rotate: -180, opacity: 0 }, animate: { rotate: 0, opacity: 1 }, transition: { duration: 0.8, delay: 0.9 + index * 0.1 }, children: _jsx(sun.icon, { className: "w-8 h-8 ".concat(sun.iconColor) }) }), _jsx(motion.p, { className: "text-2xl font-bold text-foreground mb-1", initial: { opacity: 0 }, animate: { opacity: 1 }, transition: { duration: 0.5, delay: 1 + index * 0.1 }, children: sun.value }), _jsx("p", { className: "text-sm text-muted-foreground", children: sun.title })] }, sun.id)); }) }), _jsxs(motion.div, { className: "mt-6 pt-4 border-t border-white/20 text-center", initial: { opacity: 0 }, animate: { opacity: 1 }, transition: { duration: 0.5, delay: 1.3 }, children: [_jsx("p", { className: "text-sm text-muted-foreground mb-1", children: "Day Length" }), _jsx("p", { className: "font-semibold", children: (function () {
                                    var sunrise = new Date(weatherData.sunrise);
                                    var sunset = new Date(weatherData.sunset);
                                    var dayLength = sunset.getTime() - sunrise.getTime();
                                    var hours = Math.floor(dayLength / (1000 * 60 * 60));
                                    var minutes = Math.floor((dayLength % (1000 * 60 * 60)) / (1000 * 60));
                                    ...
                                    weatherData.pressure < 1009 ? 'Low pressure' :
                                        'Normal pressure' })] }), _jsx("div", { className: "mt-4", children: _jsx("div", { className: "w-full bg-white/20 rounded-full h-2", children: _jsx(motion.div, { className: "h-2 rounded-full ".concat(weatherData.pressure > 1020 ? 'bg-green-400' :
                                    weatherData.pressure > 1013 ? 'bg-blue-400' :
                                        weatherData.pressure > 1009 ? 'bg-yellow-400' :
                                            'bg-red-400'), initial: { width: 0 }, animate: { width: "".concat(Math.min(((weatherData.pressure - 980) / 60) * 100, 100), "%") }, transition: { duration: 1, delay: 1.4 } }) }) })] })] }));
}
